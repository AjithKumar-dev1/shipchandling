...
public class StockUnitMasterController {
    ...
    @Operation(
            summary = "Update Stock unit details",
            description = "Updates the details of an existing Stock unit identified by its ID",
            responses = {
                    @ApiResponse(
                            responseCode = "200",
                            description = "Stock unit updated successfully",
                            content = @Content(
                                    mediaType = "application/json",
                                    schema = @Schema(implementation = StockUnitMasterDto.class),
                                    examples = @ExampleObject(
                                            value = "{\"status\": 200, \"message\": \"Stock unit updated successfully\", \"data\": {...}}"
                                    )
                            )
                    ),
                    @ApiResponse(
                    ...
                    @ApiResponse(
                            ...
                            content = @Content(
                                    mediaType = "application/json",
                                    examples = @ExampleObject(
                                            value = "{\"status\": 404, \"message\": \"Stock unit not found with stockUnitPoid: 123\"}"
                                    )
                            )
                    )
            }
    ...
    public ResponseEntity<?> getCountryList(@ParameterObject Pageable pageable,
                                            @RequestBody(required = false) FilterRequestDto filters, @Parameter(
                    ...
                    required = true)
                                            ...
                                            @RequestParam String actionRequested) {
        try {
            Map<String, Object> countries = stockUnitService.listStockUnits(documentId, filters, pageable);
